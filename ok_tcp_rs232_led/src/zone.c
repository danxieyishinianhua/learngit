...
#ifdef debug_malloc
msgpack_zone zone_buff;
unsigned char chunk_buff[sizeof(msgpack_zone_chunk)+2048];
#endif
...
static inline bool init_chunk_list(msgpack_zone_chunk_list* cl, size_t chunk_size)
{
	...
	#ifdef debug_malloc
		msgpack_zone_chunk* chunk=(msgpack_zone_chunk*)&chunk_buff;
	#else
